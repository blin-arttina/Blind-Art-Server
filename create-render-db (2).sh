...
set -uo pipefail
# Note: deliberately NOT using 'set -e' — a failed curl call should be
...
HTTP_STATUS=$(curl -s -o /tmp/owners.json -w "%{http_code}" \
...
OWNER_ID=$(python3 -c "
import json
with open('/tmp/owners.json') as f:
    data = json.load(f)
...
HTTP_STATUS=$(curl -s -o /tmp/create_db.json -w "%{http_code}" \
...
DB_ID=$(python3 -c "
import json
with open('/tmp/create_db.json') as f:
    data = json.load(f)
print(data.get('id', ''))
" 2>/dev/null)
...
import json
with open('/tmp/conn_info.json') as f:
    data = json.load(f)
